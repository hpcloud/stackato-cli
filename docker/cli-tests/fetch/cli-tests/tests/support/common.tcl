...
apply {{} {
	...
	STACKATO_CLI_TEST_TARGET theplaintarget api.stackato-nightly.activestate.com
	STACKATO_CLI_TEST_USER	 theuser        cli-tester@test
	STACKATO_CLI_TEST_GROUP	 thegroup       cli-test-group
	STACKATO_CLI_TEST_DRAIN	 thedrain       tcp://flux.activestate.com:11100
	STACKATO_CLI_TEST_ADMIN  adminuser      cli-test-admin@test
	STACKATO_CLI_TEST_APASS  adminpass      cli-test-admin-pass
	STACKATO_CLI_TEST_ORG    theorg         cli-test-org
	STACKATO_CLI_TEST_SPACE  thespace       cli-test-space
    } {
...
proc already {ptype pname type name} {
    ...
    return "Found a problem with $ptype \"$pname\": $lead$type named \"$name\" already exists. Please use a different name."
}
...
set post30 [expr {[package vcompare [dict get [run debug-target --target [thetarget]] Version] 3.0] > 0}]
set pre32  [expr {[package vcompare [dict get [run debug-target --target [thetarget]] Version] 3.1] < 0}]
...
tcltest::testConstraint 3.2+    $post30
tcltest::testConstraint 3.0-    $pre32
