...
# Makefiles
package provide s-http 2.7.13
package provide http   2.7.13 ;# we replace the core package, fake autoproxy and the like
...
proc http::Parray {a {pattern *}} {
    ...
    foreach name $names {
        ...
        lappend r [format "%-*s = %s" $maxl $nameString $array($name)]
    }
    join $r \n
}
...
proc http::unregister {proto} {
	...
	return -code error \
	    -errorcode [list HTTP INVALID PROTOCOL $proto] \
...
proc http::config {args} {
	...
	if {![regexp -- $pat $flag]} {
		...
		"Unknown option $flag, must be: $usage"
	}
	return $http($flag)
    } else {
...
proc ::http::CloseSocket {s {token {}}} {
    ...
    Log CloseSocket/$s/$token/
    Log "Event readable $s $token - off"
...
proc http::geturl {url args} {
    ...
    # STACKATO -- totalsize init as "" see also http::Event.
	...
	# STACKATO -- Restricted to non-https urls. https passes
	# STACKATO -- through proxy unchanged and the final
	# STACKATO -- destination should see the proper path instead
	# STACKATO -- of full url.
	...
	if {[info exists socketmap($state(socketinfo))]} {
		...
		Log "reusing socket $sock for $state(socketinfo)"
		    ...
		    Log "Event readable $sock $token - off"
		}
	    }
	}
	...
	Log "Opening: $defcmd $sockopts $targetAddr"
    ...
    Log "Event writable $sock $token - Connect $proto $phost $srvurl"
...
proc http::Connected { token proto phost srvurl} {
	...
	Log ConfigQC($state(-querychannel))=[fconfigure $state(-querychannel)]
	set contDone 0
    }
    ...
    if {[catch {
	Log "$how $srvurl HTTP/$state(-protocol)"
	Log "Accept: $http(-accept)"
	...
	puts $sock "User-Agent: $http(-useragent)"
        ...
        if {$state(-protocol) > 1.0 && !$state(-keepalive)} {
	    Log        "Connection: close" ;# RFC2616 sec 8.1.2.1
	    puts $sock "Connection: close" ;# RFC2616 sec 8.1.2.1
        }
        if {[info exists phost] && ($phost ne "") && $state(-keepalive)} {
	    Log        "Proxy-Connection: Keep-Alive"
	...
	foreach {key value} $state(-headers) {
		...
		Log "User-specified Content-Length: $value"
	    }
	...
	if {$isQuery || $isQueryChannel} {
		...
		Log        "Content-Type: $state(-type)"
		puts $sock "Content-Type: $state(-type)"
	    }
	    if {!$contDone} {
		Log        "Content-Length: $state(querylength)"
...
proc http::Write {token} {
    ...
    Log "Write($sock): $state(queryoffset) ... [expr {$state(queryoffset) + $state(-queryblocksize) - 1}]"
...
proc http::Event {sock token} {
	...
	if {[catch {gets $sock line} n]} {
	    ...
	    if {$state(-validate)} {
		Log no-body-expected
		Eof $token
		return
	    }

	    # STACKATO -- auto-close the connection on our side for a
	    # STACKATO -- response length 0, even for setting of
		...
		if {[info exists state(-channel)]} {
		    ...
		    Log ConfigC($state(-channel))=[fconfigure $state(-channel)]
		}
	    }
	    if {
	    ...
	    Log past/headers/no-copy/wait-eof
	    Log RE=[fileevent $sock readable]
	    Log WE=[fileevent $sock writable]
	} elseif {$n > 0} {
		...
		switch -- [string tolower $key] {
			...
			set requested [string trim [string tolower $value]]
			if {($state(connection) eq "keep-alive") &&
			    ($requested eq "close")
			} {
...
proc http::Gunzip {data} {
    ...
    set pos 10
    if {$magic != 0x1f8b} {
        return -code error -errorcode {HTTP GZIP EXPECTED} \
	    "invalid data: supplied data is not in gzip format"
    }
    if {$method != 8} {
        return -code error -errorcode {HTTP COMPRESSION INVALID} \
	...
	return -code error \
	    -errorcode {HTTP GZIP CHECKSUM MISMATCH} \
	    "invalid data: checksum mismatch $crc != $chk"
    }
    return $inflated
}
