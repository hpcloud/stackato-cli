...
proc ::stackato::mgr::ssh::run {config args appname instance {bg 0} {eincmd {}} {eocmd {}}} {
    ...
    set target  [ctarget get]
    set token   [auth get]
    set keyfile [targets keyfile $token]
...
proc ::stackato::mgr::ssh::copy {config paths appname instance} {
    debug.mgr/ssh {}
	...
	local/remote {
	    ...
	    CopyLocalRemote $config $appname $instance $src $dst
	}
	remote/local {
	    # Stream remote to local, taking destination path type
	    # (file, directory, missing) into account.
	    CopyRemoteLocal $config $appname $instance $src $dst
	}
	remote/remote {
...
proc ::stackato::mgr::ssh::CopyLocalRemote {config appname instance src dst} {
    debug.mgr/ssh {}
...
proc ::stackato::mgr::ssh::CopyRemoteLocal {config appname instance src dst} {
    debug.mgr/ssh {}
...
proc ::stackato::mgr::ssh::CopyLocalRemoteFileFile {src dst} {
    ...
    upvar 1 config config appname appname instance instance
...
proc ::stackato::mgr::ssh::TestIsFile {path} {
    ...
    if {![run $config [list "test -f $path"] $appname $instance 2]} {
	return 1
    } else {
	return 0
    }
...
proc ::stackato::mgr::ssh::TestIsDirectory {path} {
    ...
    # (!= 0) == true  ==> FAIL
    set path [quote1 $path]
    if {![run $config [list "test -d $path"] $appname $instance 2]} {
	return 1
    } else {
	return 0
    }
...
proc ::stackato::mgr::ssh::TestExists {path} {
    ...
    # (!= 0) == true  ==> FAIL
    set path [quote1 $path]
    if {![run $config [list "test -e $path"] $appname $instance 2]} {
	return 1
    } else {
	return 0
    }
