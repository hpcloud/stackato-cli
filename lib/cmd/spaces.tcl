...
proc ::stackato::cmd::spaces::list {config} {
	...
	foreach space [[corg get] @spaces get 2] {
	    ...
	    lappend values [join [$space @apps @name] \n]
	    lappend values [join [$space @service_instances @name] \n]

	    if {$full} {
		lappend values [join [$space @developers email] \n]
		lappend values [join [$space @managers   email] \n]
		lappend values [join [$space @auditors   email] \n]
	    }
...
proc ::stackato::cmd::spaces::show {config} {
	...
	$t add Apps         [join [$space @apps @name] \n]
	$t add Services     [join [$space @service_instances @name] \n]
	$t add Domains      [join [$space @domains @name] \n]

	if {[$config @full]} {
	    $t add Developers [join [$space @developers email] \n]
	    $t add Managers   [join [$space @managers   email] \n]
	    $t add Auditors   [join [$space @auditors   email] \n]
	}
